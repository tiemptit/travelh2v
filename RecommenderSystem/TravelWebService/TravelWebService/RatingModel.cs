...
namespace TravelWebService
{
    public class RatingModel
    {
        ...
        public bool Rate()
        {
            ...
            if (id_user == -1)
            {
                DbHelper.RunScripts(string.Format("insert into users values ('" + username + "','123',1989,1)"));
                id_user = checkIfUserExisted();
                return DbHelper.RunScripts(string.Format("insert into real_ratings values (" + id_user + "," + id_place + "," + id_temperature + "," + id_companion + "," + id_farmiliarity + "," + id_mood + "," + id_budget + "," + id_weather + "," + id_travel_length + "," + time + "," + rating + ")"));
            }
            else 
            { 
                ...
                if (id_real_rating != -1) 
                {
                    return DbHelper.RunScripts(string.Format("update real_ratings set rating = " + rating + " where id = " + id_real_rating));
        ...
        {
            DataTable dt = DbHelper.RunScriptsWithTable(string.Format("select id from users where email ='" + username + "'"));
            if (dt.Rows.Count != 0)
            {
